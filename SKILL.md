...
- For named direct port-to-port signals, always set `schDisplayLabel` on the `<trace />` so the schematic shows the intended signal name instead of an autogenerated fallback label. Example: `<trace from="R1.pin1" to="U1.SENSE" schDisplayLabel="SENSE" />`. Do not add redundant `schDisplayLabel` to simple `net.GND`, `net.VCC`, or other net connections unless the displayed schematic label must differ from the net name.
