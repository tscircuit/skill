...
- Use `<trace />` for connectivity; every trace must have a concise, stable `name` prop because it may appear as a visible net label when routing cannot draw the trace. Choose a meaningful circuit-specific net or signal name; avoid long endpoint slugs and generic placeholders that do not describe the signal. Prefer net connections (`net.GND`, `net.VCC`, etc.) for power/ground.
